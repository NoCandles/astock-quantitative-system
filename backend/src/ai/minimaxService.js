...
export async function analyzeStockWithMinimax(data, apiKey) {
  const { symbol, name, quote, technicalIndicators, klineDaily } = data;
  console.log('开始调用MiniMax API...');
  const configApiKey = process.env.MINIMAX_API_KEY;
  ...
  if (!finalApiKey || finalApiKey === 'your_minimax_api_key_here') {
  ...
  try {
    const response = await axios.post(
      `${baseUrl}/chat/completions`,
      {
        ...
        messages: [
          {
            role: 'system',
            content: '你是专业的A股技术分析师，具备扎实的技术分析能力和丰富的实战经验。你需要以专业、客观的态度分析股票，给出基于技术面的判断。请用简洁专业的语言回答，结论要明确有理有据。永远记住：股市有风险，分析仅供参考，不构成投资建议。',
          },
          {
            role: 'user',
            content: prompt,
    ...
    return response.data.choices?.[0]?.message?.content || '分析服务暂时无法返回结果，请稍后重试。';
  } catch (error) {
